...
await sharp(svg).resize(180, 180).flatten({ background: "#1E9E4A" }).png().toFile(out("apple-touch-icon.png"));

// Maskable: ícono cuadrado sin esquinas redondeadas y con zona segura
const maskable = Buffer.from(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><rect width="512" height="512" fill="#1E9E4A"/>
  <g transform="translate(76.8 76.8) scale(0.7)">${svg.toString().replace(/<svg[^>]*>|<\/svg>/g, "").replace(/<rect width="512" height="512" rx="112" fill="#1E9E4A"\/>/, "")}</g></svg>`
);
